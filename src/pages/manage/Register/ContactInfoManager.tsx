...
function ContactInfoManager() {
    ...
    const [countrycode, setContrycode] = useState(stateForm?.country_code || '+84');
        ...
        <div style={{display:"flex", justifyContent:"center", backgroundColor:"#fff", paddingTop:40}}>
                <Space direction="vertical" style={{width:350}}>
                    ...
                    <Form layout="vertical" onFinish={onFinishContact}>
                        <Form.Item initialValue={stateForm?.firstname} name={"firstname"} label={<span style={{fontWeight:600}}>Tên</span>}
                        rules={[
                        ...
                        <Form.Item initialValue={stateForm?.lastname} name={"lastname"} label={<span style={{fontWeight:600}}>Họ</span>}
                        rules={[
                            {required: true, message: 'Vui lòng nhập tên của bạn' },
                            { min: 2, message: 'Tên của bạn phải ít nhất 2 ký tự' },
                            { max: 20, message: 'Tên của bạn phải nhiều nhất 10 ký tự' },
                        ...
                        <Form.Item initialValue={stateForm?.phonenumber} name={"phonenumber"}
                         rules={[
                            {required: true, message: 'Vui lòng nhập số điện thoại' },
                            {
                                validator: (_, value) => {
                                    ...
                                    return Promise.reject(new Error('Số điện thoại không hợp lệ (10 - 12 số)'));
                                },
                            }
                        ]}>
                            <Space.Compact>
                                <Input style={{ width: '20%' }} value={countrycode} defaultValue={countrycode} />
                                <Input defaultValue={stateForm?.phonenumber} style={{ width: '80%' }} placeholder="Nhập số điện thoại ..." />
                            </Space.Compact>
                        </Form.Item>
                        ...
                        <Form.Item style={{marginTop:5}}>
                            <Button htmlType="submit" type="primary" style={{width:"100%", padding:18}}>Tiếp theo</Button>
                        </Form.Item>
                    </Form>
                    <Privacy_Policy />
                </Space>
