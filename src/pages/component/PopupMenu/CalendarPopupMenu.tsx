...
function CalendarPopupMenu() {
    const navigate = useNavigate();
    const token = localStorage.getItem('token');
    return ( 
         <div className="calendarpopupmenu" style={{marginTop:5,position:"absolute", top:"100%", left:0, color:"#000",
            backgroundColor:"#fff", display:"none"
         }}>
            <ul style={{ padding: 0, margin: "0px 0px 5px 0px", listStyle: "none", width:250, 
                boxShadow: "0px 4px 10px rgba(0, 0, 0, 0.2)"
            }}>
                <li onClick={()=>navigate(`/manage/calendar-and-room/calendar?token=${token}`)} style={{ padding: "10px 10px", lineHeight: "1.4", fontSize: "14px" }}>Lịch</li>
